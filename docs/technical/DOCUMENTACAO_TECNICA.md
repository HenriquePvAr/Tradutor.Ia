...
| Retomada de job interrompido | **IMPLEMENTADO** — TDD #56: `POST /api/ui/resume` mais o painel `#interruptedJobsPanel` e o botão `Retomar`, renderizado apenas para os jobs que o backend marca `can_resume` (`static/tradutor_ui.js`, `test_interrupted_job_resume_ui.py`/`.mjs`) |
...
| `BETA-AUTHORIZER-NOT-WIRED` | Alta (bloqueia Beta externa) | `ui_bridge.py` instancia `LocalDevelopmentBetaAuthorizer()` fixo no construtor; `beta_license.build_beta_license_authorizer` (que honra `BETA_LICENSE_PROVIDER=supabase`) não tem nenhum chamador de produção, e `BETA_LICENSE_PROVIDER` não aparece em `.env.example`. Na prática o gate autoriza sempre, mesmo com entitlement remoto configurado. | `ui_bridge.py:399`, `beta_license.py:312`, `.env.example` | Missão de auth/licença Beta: selecionar o authorizer por configuração e documentar a variável. **Não corrigir durante o Quality Freeze sem missão dedicada** |
