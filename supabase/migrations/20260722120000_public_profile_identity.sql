...
alter table public.profiles
    add constraint profiles_status_check check (status in ('online', 'away', 'busy', 'offline')),
    add constraint profiles_public_role_len check (char_length(public_role) <= 80),
    add constraint profiles_pronouns_len check (char_length(pronouns) <= 30),
    add constraint profiles_status_message_len check (char_length(status_message) <= 120),
    add constraint profiles_accent_color_format check (accent_color ~ '^#[0-9a-fA-F]{6}$');
